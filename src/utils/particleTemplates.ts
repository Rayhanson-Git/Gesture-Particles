...
const generateAuroraPositions = (count: number): Float32Array => {
  const positions = new Float32Array(count * 3);
  const curtains = 4;
  const width = 5;
  const height = 3;
  ...
  for (let i = 0; i < count; i++) {
    const curtainIndex = Math.floor(Math.random() * curtains);
    const curtainOffset = (curtainIndex / curtains - 0.5) * depth;

    const t = Math.random();
    const x = (t - 0.5) * width;

    const wave1 = Math.sin(t * Math.PI * 3 + curtainIndex * 1.2) * 0.6;
    const wave2 = Math.sin(t * Math.PI * 5 + curtainIndex * 0.8) * 0.3;
    const wave3 = Math.sin(t * Math.PI * 7 + curtainIndex * 2.1) * 0.15;

    const heightFalloff = Math.random();
    const verticalBias = Math.pow(heightFalloff, 0.6);
    const y = verticalBias * height - height * 0.3;

    const lateralWave = (wave1 + wave2 + wave3) * (1 - verticalBias * 0.5);
    ...
    positions[i * 3 + 1] = y + lateralWave + spreadY;
    positions[i * 3 + 2] = curtainOffset + wave2 * 0.4 + spreadZ;
  }
...
export const particleTemplates: ParticleTemplate[] = [
  ...
  },
  {
    id: 'aurora',
    name: 'Aurora',
    generatePositions: generateAuroraPositions
  }
];
